...
int main(void){
  ...
  DELAY_Ms(2000);
    ...
    while(!(ADC1->SR & (1<<1)));
		...
		if(adc_result1 > BENT_Threshold && accel_x < -5 && last_cmd != '0'){ //Stop
		...
		else if(adc_result1 < BENT_Threshold && accel_x < -5  && last_cmd != '1'){ //FWD
			UART_Send("AT+CIPSEND=1\r\n");
			DELAY_Ms(100);
			cmd = '1';
			UART_Send("1");
		...
		else if(adc_result1 > BENT_Threshold && accel_z > 5  && last_cmd != '3'){ //Right
			UART_Send("AT+CIPSEND=1\r\n");
			DELAY_Ms(100);
			cmd = '3';
			UART_Send("3");
		...
		else if(adc_result1 > BENT_Threshold && accel_z < -5  && last_cmd != '4'){ //Left
			UART_Send("AT+CIPSEND=1\r\n");
			DELAY_Ms(100);
			cmd = '4';
			UART_Send("4");
		...
		else if(adc_result1 < BENT_Threshold && accel_z > 5  && last_cmd != '5'){//FWD Right
			UART_Send("AT+CIPSEND=1\r\n");
			DELAY_Ms(100);
			cmd = '5';
			UART_Send("5");
		...
		else if(adc_result1 < BENT_Threshold && accel_z < -5 && last_cmd != '6'){//FWD Left
			UART_Send("AT+CIPSEND=1\r\n");
			DELAY_Ms(100);
			cmd = '6';
			UART_Send("6");
